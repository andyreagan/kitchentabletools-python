...
letters = [x.upper() for x in ["a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l", "m", "n", "o", "p", "q", "r", "s", "t", "u", "v", "w", "x", "y",
                               "z", "aa", "bb", "cc", "dd", "ee", "ff", "gg", "hh", "ii", "jj", "kk", "ll", "mm", "nn", "oo", "pp", "qq", "rr", "ss", "tt", "uu", "vv", "xx", "yy", "zz"]]
...
def crop_shift_top(pdffile):
    ...
    command = "gs -o {0}-topcropped.pdf -sDEVICE=pdfwrite -c \"[/CropBox [0 0 800 425]\" -c \" /PAGES pdfmark\" -f {0}.pdf".format(
        pdffile.replace(".pdf", ""))
    # 477
...
def listify(raw_text, lang="en"):
    ...
    words = [x.lower() for x in re.findall(
        r"(?:[0-9][0-9,\.]*[0-9])|(?:http[s]*://[\w\./\-\?\&\#]+)|(?:[\w\@\#\'\&\]\[]+)|(?:[b}/3D;p)|’\-@x#^_0\\P(o:O{X$[=<>\]*B]+)", raw_text, flags=re.UNICODE)]
